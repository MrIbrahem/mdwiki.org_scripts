...
def mock_make_title(monkeypatch: pytest.MonkeyPatch):
    ...
    # monkeypatch.setattr("flask_app.main_app.public_jobs_workers.fixref.fixref_text_new.make_title", lambda x: title)
    monkeypatch.setattr("flask_app.main_app.public_jobs_workers.fixref.make_title_bot.get_url", lambda x: result)

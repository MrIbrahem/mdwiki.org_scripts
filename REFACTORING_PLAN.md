...
- [ ] **Dead API wrapper** — `flask_app/main_app/api_services/query_api.py::get_template_pages_newapi()` (line 10) references `api.NewApi()` which doesn't exist. The function is unused anywhere.
...
- [ ] Remove dead code: `get_template_pages_newapi()` from `query_api.py`
...
*Generated by Claude Refactoring Analyst after comprehensive codebase analysis.*
